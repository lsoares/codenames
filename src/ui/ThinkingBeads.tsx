...
// minute; the current minute grows from small to full and blinks, so progress is
// felt without a ticking number. A soft beep marks each whole minute. At ten
// minutes it stops and greys out. Purely client-local: nobody else sees or hears
...
export default function ThinkingBeads(props: { team: Team }) {
  ...
  useEffect(() => {
    if (completed > 0) playSound('minute')
  }, [completed])
  ...
  return (
    ...
    >
      ...
      {!capped && (
        // Keyed on the minute so each new minute mounts a fresh small dot that grows
        // in, rather than the previous one shrinking back down.
        <span
          key={completed}
          className={`${styles.dot} ${styles.growing}`}
          style={{ transform: `scale(${0.28 + 0.72 * fraction})` }}
        />
      )}
    </span>
  )
