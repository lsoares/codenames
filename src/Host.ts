...
export class Host implements Session {
  ...
  static start(faces: string[], startingTeam: 'red' | 'blue', mode: BoardMode): Promise<Host> {
    return Host.launch(randomCode(), createGame(faces, startingTeam, mode), 'new', 4)
  }
  ...
  // Leaving for good: stop the heartbeat and drop the peer. Destroying the peer
  // closes every guest connection, so guests fall into their usual FIFO takeover.
  close(): void {
